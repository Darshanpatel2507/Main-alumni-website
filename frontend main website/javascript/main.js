/* =====================================================
   ALUMNIHUB — MAIN.JS (COMPLETE FULL VERSION)
   ===================================================== */
...
function initFilters() {
  ...
  chips.forEach((chip) => {
    chip.addEventListener("click", () => {
      if (document.getElementById('dashboard-views').style.display === 'block') return;
...
function initModal() {
  ...
  if (openSigninBtn && signinModal) openSigninBtn.addEventListener("click", () => signinModal.classList.add("open"));
  if (closeSigninBtn && signinModal) closeSigninBtn.addEventListener("click", () => signinModal.classList.remove("open"));
  if (closeSignupBtn && signupModal) closeSignupBtn.addEventListener("click", () => signupModal.classList.remove("open"));
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { name, role, branch, passing_year: passingYear ? Number(passingYear) : null }
        }
      });
  ...
  if (reportModalBtn) reportModalBtn.addEventListener('click', () => document.getElementById("report-modal").classList.remove("open"));
  
  if (reportForm) {
    reportForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const reason = document.getElementById("report-reason").value;
      const reportedId = window._currentReportId;
      const reportType = window._currentReportType;
...
async function handleSession(session) {
  ...
  if (userData) {
    if (userData.approved === false && userData.role !== 'admin') {
      ...
      showToast("Your account is restricted.");
      await supabase.auth.signOut();
      currentUser = null;
    } else {
...
function hideDashboard() {
   ...
   if (document.getElementById("alumni-dashboard")) document.getElementById("alumni-dashboard").style.display = 'none';
  ...
  if (eventForm) {
    eventForm.addEventListener("submit", async (e) => {
      ...
      const { error } = await supabase.from('events').insert({
        title, description: desc, date, time, location: loc, type, created_by: currentUser.id
      });
  ...
  if (postForm) {
    postForm.addEventListener("submit", async (e) => {
      ...
      const { error } = await supabase.from('discussions').insert({
        title, content, tag, user_id: currentUser.id
      });
...
// Global Edit Action
window.editContent = async function(type, id) {
...
window.deleteOwnContent = async function(type, id) {
...
window.deleteContent = async function(type, contentId, reportId) {
...
window.updateConn = async function(connId, status) {
...
window.openChat = async function(studentId, alumniId, peerName, userType) {
